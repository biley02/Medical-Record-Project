...
const universityMail = (data, host, protocol) => {
    const maxAge = 3 * 60 * 60
...
const nomineeMail = (ticket, nominee, user, host, protocol) => {
  ...
  var mailOptions = {
    ...
    subject: "Give access to hospital",
    html:
      "Hello,<br> You are nominee of " +
      patient_name +
      ".<br>Kindly grant access to patient data. <br> <a href=" +
      link +
      ">Click here </a>",
  };
  transporter.sendMail(mailOptions, function (error, info) {
...
const doctorRegistrationMail = (doctor) => {
  var transporter = nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 465,
    secure: true,
  ...
  const userContent = `Hello, Dr. ${doctor.name}.<br> 
    We have registered your details into our database - <br>
    You can now login into our website and update your details using the details given below: <br>
    email id: ${doctor.email} <br>
    password(temporary) : ${doctor.password} <br>
    Thank you!`;

  var mailOptions = {
    from: process.env.NODEMAILER_EMAIL,
    to: `${doctor.email}`,
    subject: `Hello ${doctor.name}, welcome to our website`,
    html: userContent,
  };
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
